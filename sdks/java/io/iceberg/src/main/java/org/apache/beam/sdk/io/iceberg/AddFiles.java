  ...
  public PCollectionRowTuple expand(PCollection<String> input) {
    if (input.isBounded().equals(UNBOUNDED)) {
      Preconditions.checkArgument(
          evolution.getIncompatibleSchemaHandling()
              != SchemaEvolutionConfig.IncompatibleSchemaHandling.FAIL_PIPELINE,
          "FAIL_PIPELINE is batch only: in streaming a failing window's schema commit would be"
              + " retried forever and hold every later window. Use ROUTE_TO_ERRORS or leave the"
              + " handling unset.");
