...
class DryRunReport extends DoFn<List<CollectDistinctSchemas.SchemaGroup>, Row> {
      ...
      if (table == null) {
        ...
        SchemaDelta delta =
            SchemaDelta.classify(
                table,
                FileSchemas.markRequired(SchemaParser.fromJson(json), group.getNullFreeColumns()));
  ...
  /** With no table, every column of the schema would be created optional at every level. */
  private List<String> createChanges(String json) {
    org.apache.iceberg.Schema seed = CommitSchemaUnion.relaxAll(SchemaParser.fromJson(json));
