...
public class BigQueryUtils {
  ...
  public abstract static class SchemaConversionOptions implements Serializable {
    ...
    /**
     * Controls how BigQuery {@code TIMESTAMP(12)} (picosecond precision) columns are converted to
     * Beam schema types.
     *
     * <p>Since Beam's datetime types only support up to nanosecond precision, this option specifies
     * whether to truncate to microseconds, truncate to nanoseconds, or preserve full precision as a
     * string.
     *
     * <p>This option has no effect on {@code TIMESTAMP(6)} (microsecond).
     *
     * <p>Defaults to {@link TimestampPrecision#NANOS}.
     */
    public abstract TimestampPrecision getPicosecondTimestampConversion();

    public static Builder builder() {
      return new AutoValue_BigQueryUtils_SchemaConversionOptions.Builder()
          .setInferMaps(false)
          .setPicosecondTimestampConversion(TimestampPrecision.NANOS);
    }
    ...
    public abstract static class Builder {
      ...
      public abstract Builder setPicosecondTimestampConversion(TimestampPrecision conversion);
  ...
  private static FieldType fromTableFieldSchemaType(
    ...
    switch (typeName) {
      ...
      case "TIMESTAMP":
        ...
        if (options.getPicosecondTimestampConversion() == TimestampPrecision.MICROS) {
          return FieldType.logicalType(Timestamp.MICROS);
        }
        if (options.getPicosecondTimestampConversion() == TimestampPrecision.NANOS) {
          return FieldType.logicalType(Timestamp.NANOS);
        }
        if (options.getPicosecondTimestampConversion() == TimestampPrecision.PICOS) {
          return FieldType.STRING;
        }
      ...
      if (JSON_VALUE_PARSERS.containsKey(fieldType.getTypeName())) {
        ...
        try {
          if (jsonBQString.endsWith(" UTC")) {
            // Remove " UTC" suffix and replace with "Z" for ISO format
