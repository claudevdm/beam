...
def _verify_dill_compat():
  if not dill:
    raise RuntimeError("Error importing dill for encoding deterministic special" \
                       "dill types. Ensure dill version '0.3.1.1' is installed" \
                       "in the execution environment.")
  if dill.__version__ != "0.3.1.1":
    raise RuntimeError("Error verifying dill compatibility forencoding deterministic special" \
                      "dill types. Ensure dill version '0.3.1.1' is installed" \
                      f"in the execution environment. Found dill version '{dill.__version__}")
...
class FastPrimitivesCoderImpl(StreamCoderImpl):
  ...
  def encode_type_266(self, t, stream):
    _verify_dill_compat()
  ...
  def encode_type(self, t, stream):
    ...
    if self.force_use_dill:
      return self.encode_type_266(t, stream)
    bs = cloudpickle_pickler.dumps(
        t, config=cloudpickle_pickler.NO_DYNAMIC_CLASS_TRACKING_CONFIG)
    # Ensure we always use the first def in the dynamic types case so that
    # regular python pickler works. Doing this during encoding is necessary
    # in DirectRunner case.
    if not _unpickled_types.get(bs, None):
      _unpickled_types[bs] = t
    stream.write(bs, True)

  def decode_type(self, stream):
    if self.force_use_dill:
      return _unpickle_type_266(stream.read_all(True))
...
def _unpickle_named_tuple_266(bs, items):
  return _unpickle_type_266(bs)(*items)
