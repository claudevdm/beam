...
3. Merging same-outer types recursively (e.g., List[int] | List[str] -> List[int|str])
4. Handling different tuple arities via len() discrimination

See experiments/beam_knowledge_base/028_Union_Coders.md for design details.
"""
...
class ArityDiscriminatingTupleCoder(FastCoder):
  ...
  def _create_impl(self):
    return ArityDiscriminatingTupleCoderImpl(
        {arity: coder.get_impl() for arity, coder in self._arity_to_coder.items()},
  ...
  def is_deterministic(self) -> bool:
    return (
        ...
        (self._fallback_coder is None or self._fallback_coder.is_deterministic()))
  ...
  def __repr__(self):
    return 'ArityDiscriminatingTupleCoder[%s]' % ', '.join(
        f'{arity}: {coder}' for arity, coder in sorted(self._arity_to_coder.items()))
...
# iterables. This is controlled by the flag `_iterable_coder_uses_abstract_iterable_by_default`
# which is set to False (see coder_impl.py:1449).
#
# The decoding flow is:
# 1. SequenceCoderImpl.decode_from_stream (coder_impl.py:1350-1376) reads elements
#    into a Python list: `elements = [...]` (line 1355) or `elements = []` with
#    `elements.append(...)` (lines 1360-1364).
# 2. IterableCoderImpl._construct_from_sequence (coder_impl.py:1462-1466) returns
#    the list directly when `_use_abstract_iterable` is False (the default).
#
...
# for runtime type discrimination since `type([1,2,3]) is collections.abc.Iterable`
# is False.
...
def _get_outer_type(typehint) -> Optional[type]:
  ...
  """
  # For concrete types (int, str, custom classes, NamedTuples), return the type itself
  if isinstance(typehint, type):
    return typehint
  # Look up TypeConstraint class in mapping
  # Note: beam.Row types (RowTypeConstraint) are not in the mapping and return None,
  # because at runtime all beam.Row values have type Row regardless of schema.
...
def build_smart_union_coder(
  ...
  # Deduplicate type hints while preserving order
  seen = set()
  unique_hints = []
  for hint in type_hints:
    # Use repr for hashability of TypeConstraints
    hint_key = repr(hint) if hasattr(hint, '__class__') else hint
    if hint_key not in seen:
      seen.add(hint_key)
      unique_hints.append(hint)
  type_hints = unique_hints
...
def _merge_same_outer_types(
  ...
  if outer_type == list:
    ...
    # Recursively build union coder for element types
  ...
  if outer_type == dict:
    # Dict[K1, V1], Dict[K2, V2] -> MapCoder(UnionCoder[K1, K2], UnionCoder[V1, V2])
    key_types = []
    value_types = []
    for hint in hints:
    ...
    key_coder = build_smart_union_coder(key_types, registry)
    value_coder = build_smart_union_coder(value_types, registry)
...
def _merge_tuple_positions(tuple_hints: List[Any], registry) -> Coder:
  ...
  for hint in tuple_hints:
    ...
    if len(hint.tuple_types) != arity:
      raise ValueError(f"Arity mismatch: expected {arity}, got {len(hint.tuple_types)}")
...
from apache_beam.coders import typecoders
typecoders.registry.register_coder(typehints.UnionConstraint, _SmartUnionCoderFactory)
