...
class PipelineConstructionOptionsTest(unittest.TestCase):
  def test_nested_scoping(self):
    ...
    with scoped_pipeline_options(outer_options):
      ...
      # After inner scope exits, outer options should be restored
      self.assertIs(get_current_pipeline_options(), outer_options)
  ...
  def test_thread_isolation(self):
    ...
    def thread_worker(thread_id, runner_name):
      try:
        ...
        with scoped_pipeline_options(options):
          # Wait for all threads to enter their scoped context
          barrier.wait(timeout=5)

          # Verify we see our own options
          current = get_current_pipeline_options()
          results[thread_id] = current.get_all_options()['runner']

          # Small delay to allow potential race conditions to manifest
          import time
          time.sleep(0.01)
    ...
    # Start threads with different options
...
class PipelineIntegrationTest(unittest.TestCase):
  """Integration tests verifying options are accessible during pipeline
  construction."""
  ...
  def test_coder_sees_correct_options_during_run(self):
    ...
    # WeakRefDict supports weak references (required by shared.Shared)
    ...
    class GbkTestKey:
      ...
      def __eq__(self, other):
        return isinstance(other, GbkTestKey) and self.value == other.value
    ...
    class OptionsCapturingKeyCoder(coders.Coder):
      ...
      # shared.Shared() survives pickling - the UUID key is preserved
      ...
      def decode(self, encoded):
        return GbkTestKey(encoded.decode('utf-8'))

      def is_deterministic(self):
        # Return False to trigger as_deterministic_coder during proto conversion
        return False

      def as_deterministic_coder(self, step_label, error_message=None):
        # Capture pipeline options using shared dict
    ...
    beam.coders.registry.register_coder(GbkTestKey, OptionsCapturingKeyCoder)

    # Pre-initialize and hold reference to prevent GC
    results = OptionsCapturingKeyCoder.shared_handle.acquire(WeakRefDict)
    ...
    def construct_pipeline(worker_id):
      try:
        ...
        with beam.Pipeline(options=options) as p:
          _ = (
              p
              | beam.Create([(GbkTestKey(1), 'a'), (GbkTestKey(2), 'b')])
              | beam.GroupByKey())
  ...
  def test_barrier_inside_default_type_hints(self):
    ...
    This tests the REAL Pipeline code path (not manual scoping).
    """

    results = {}
    errors = []
    # Barrier for 2 threads to synchronize inside default_type_hints
    ...
    class BarrierTransform(beam.PTransform):
      ...
      def default_type_hints(self):
        ...
        # Both threads are now inside scoped_pipeline_options (via
        # Pipeline.apply)
        self.barrier.wait(timeout=5)

        # Now read options - this is the critical test
        opts = get_current_pipeline_options()
        if opts is not None:
          job_name = opts.get_all_options().get('job_name')
    ...
    self.assertEqual(errors, [], f"Errors occurred: {errors}")
    # Each worker should see its own job_name, not the other's
    self.assertEqual(
        results.get(1),
        'barrier_job_1',
