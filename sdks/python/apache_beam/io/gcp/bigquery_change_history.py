...
Architecture:
  1. Custom polling SDF emits _QueryRange instructions.
...
@dataclasses.dataclass
class _QueryRange:
  """Emitted by the polling SDF.

  Contains the time range to query..
  """
  chunk_start: float
  chunk_end: float
...
class _StreamRestriction:
  """Restriction carrying BQ Storage stream names.
  """
...
class _CleanupTempTablesFn(beam.DoFn):
  ...
  When streams_read >= total_streams, deletes the temp table.
  """
  ...
  def process(self, element, streams_read=beam.DoFn.StateParam(STREAMS_READ)):
...
class ReadBigQueryChangeHistory(beam.PTransform):
  ...
  def expand(self, pbegin):
    ...
    # BQ query runs so that query retries are deterministic.
    # TODO(cvandermerwe): Consider using deterministic temp table name
    # so retries can first check if it already exists.
    query_results = (
        query_ranges
