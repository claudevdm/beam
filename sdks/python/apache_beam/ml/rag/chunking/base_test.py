...
class WordSplitter(beam.DoFn):
    def process(self, element):      
        ...
        for i, word in enumerate(words):
            yield Chunk(
                content=ChunkContent(text=word),
                index=i,
...
def chunk_equals(expected, actual):
    ...
    return (expected.index == actual.index and 
...
class ChunkingTransformProviderTest(unittest.TestCase):
    ...
    def test_chunking_transform(self):
        ...
        with TestPipeline() as p:
            ...
            expected = [
                Chunk(
                    content=ChunkContent(text="hello"),
                    index=0,
                    metadata={'source': 'test.txt'}
                ),
                Chunk(
                    content=ChunkContent(text="world"),
                    index=1,
                    metadata={'source': 'test.txt'}
                ),
                Chunk(
                    content=ChunkContent(text="test"),
                    index=2,
                    metadata={'source': 'test.txt'}
                )
            ]
