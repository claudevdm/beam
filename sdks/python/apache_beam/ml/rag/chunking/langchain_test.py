...
def chunk_equals(expected, actual):
    ...
    return (
        ...
        expected.index == actual.index and
...
class LangChainChunkingTest(unittest.TestCase):
    ...
    def test_single_metadata_field(self):
        """Test chunking with one metadata field."""
        splitter = CharacterTextSplitter(chunk_size=100, chunk_overlap=20)
        provider = LangChainChunkingProvider(
            document_field='content',
        ...
        with TestPipeline() as p:
            ...
            assert_that(chunks, 
                       lambda x: all(c.metadata == {'source': 'simple.txt'} for c in x))
    ...
    def test_recursive_splitter_no_overlap(self):
        ...
        with TestPipeline() as p:
            ...
            assert_that(chunks,
                       lambda x: all(len(c.content.text) <= 30 for c in x))

    def test_recursive_splitter_with_overlap(self):
        """Test RecursiveCharacterTextSplitter with overlap."""
        splitter = RecursiveCharacterTextSplitter(
            chunk_size=30,
            chunk_overlap=10,
            separators=[". "]
    ...
    def test_recursive_splitter_custom_separators(self):
        """Test RecursiveCharacterTextSplitter with custom separators."""
        splitter = RecursiveCharacterTextSplitter(
            chunk_size=30,
            chunk_overlap=5,
            separators=["\n\n", "\n", ". ", " "]
